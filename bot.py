...
def set_max_attack_time(msg):
    ...
    args = msg.text.split()
    if len(args) != 2:
        bot.reply_to(msg, "⚠️ Usage: /second 10-600\n📌 Example: /second 180\n📌 Example: /second 300")
        return
    
    try:
        new_max = int(args[1])
        if new_max < 10 or new_max > 600:
            bot.reply_to(msg, "❌ Value must be between 10 and 600 seconds!")
            return
    except:
    ...
    bot.reply_to(msg, f"✅ MAX ATTACK TIME UPDATED!\n\n⏱️ New Max Attack Time: {MAX_ATTACK_TIME}s")
...
def attack_group(msg):
    ...
    try:
        ...
        if duration < 10 or duration > MAX_ATTACK_TIME:
            bot.reply_to(msg, f"❌ Duration must be 10-{MAX_ATTACK_TIME} seconds!")
            return
    except:
